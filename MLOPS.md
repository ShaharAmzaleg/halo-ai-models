...
2. **סקור ואשר:** פתח `pending_feedback.csv`. לכל שורה מלא את עמודת `decision`:
   - `1` = פוגעני (אשר/תקן), `0` = לא פוגעני (אשר/תקן), **ריק = דחה** (לא נכנס לאימון).
   - `suggested_label` מראה מה ההורה סימן. commit את הקובץ.
